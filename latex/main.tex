...
% JMLR style file - required for submission
\usepackage{jmlr2e}
...
\jmlrheading{1}{2025}{1-\pageref{LastPage}}{11/25}{XX/XX}{25-XXXX}{Author Names}

% Short headings for running header
\ShortHeadings{Sovereign Federated Learning with Byzantine-Resilient Aggregation}{Authors}
\firstpageno{1}
...
\author{\name Almas Ospanov \email ospanov_ad_4@enu.kz \\
       \addr Department of Computer Science\\
       L.N. Gumilev Eurasian National University\\
       Astana, Kazakhstan\\
       \AND
       \name Author Two \email author2@institution.org \\
       \addr Research Institute\\
       Institution Name\\
       City, Country}
...
Federated learning \citep{mcmahan2017communication} has emerged as a promising paradigm for training machine learning models across distributed data sources without centralizing raw data. However, existing federated learning frameworks face three critical limitations when applied to national-scale AI infrastructure:

\begin{enumerate}
    \item \textbf{Communication Inefficiency}: Standard federated averaging requires transmitting full model gradients, creating prohibitive bandwidth requirements for geographically distributed infrastructure \citep{konecny2016federated}.
    
    \item \textbf{Byzantine Vulnerability}: Classical aggregation schemes assume honest participants, leaving systems vulnerable to adversarial manipulation---a critical concern for public AI infrastructure \citep{blanchard2017machine}.
    
    \item \textbf{Provenance Opacity}: Existing frameworks lack mechanisms for verifying model training history, creating challenges for regulatory compliance and public trust \citep{hynes2018efficient}.
\end{enumerate}
...
Federated learning was introduced by \citet{mcmahan2017communication} as FedAvg, enabling collaborative model training without centralizing data. Subsequent work has addressed various challenges including communication efficiency \citep{konecny2016federated, li2020federated}, systems heterogeneity \citep{li2020convergence}, and statistical heterogeneity from non-i.i.d. data \citep{zhao2018federated, karimireddy2020scaffold}.

Communication compression techniques include gradient sparsification \citep{aji2017sparse}, quantization \citep{alistarh2017qsgd}, and error feedback mechanisms \citep{stich2019error}. \citet{hamer2020fedboost} proposed FedBoost for communication-efficient boosting, while \citet{rothchild2020fetchsgd} introduced FetchSGD using count sketches.
...
Byzantine fault tolerance in distributed learning has received considerable attention following \citet{blanchard2017machine}, who introduced Krum for robust aggregation. Subsequent work includes coordinate-wise median \citep{yin2018byzantine}, trimmed mean \citep{yin2018byzantine}, and Bulyan \citep{guerraoui2018hidden}.
...
Differential privacy \citep{dwork2006calibrating} provides rigorous privacy guarantees for machine learning. In federated settings, \citet{mcmahan2018learning} introduced DP-FedAvg, while \citet{geyer2017differentially} analyzed privacy amplification from subsampling. Secure aggregation protocols \citep{bonawitz2017practical} prevent the server from observing individual updates.
...
Blockchain technology has been applied to machine learning for model marketplaces \citep{kuo2019modelchain}, training verification \citep{hynes2018efficient}, and incentive mechanisms \citep{allen2023exchange}. In federated learning contexts, \citet{kim2019blockchained} proposed blockchain-based FL architectures, while \citet{li2020blockchain} addressed data sharing.
...
This bound is necessary for meaningful robust aggregation \citep{blanchard2017machine}.
...
\textbf{Baselines:} FedAvg \citep{mcmahan2017communication}, FedProx \citep{li2020federated}, SCAFFOLD \citep{karimireddy2020scaffold}, and Byzantine-resilient variants: Krum \citep{blanchard2017machine}, Trimmed Mean \citep{yin2018byzantine}.
...
\begin{axis}[
    ...
    xmode=log,
    log basis x=2,
    xtick={50, 100, 200, 500, 1000},
    xticklabels={50, 100, 200, 500, 1000},
]
\addplot[color=blue, mark=square] coordinates {
    (50, 2.1) (100, 2.8) (200, 3.9) (500, 6.2) (1000, 9.8)
};
\addlegendentry{FedAvg}
\addplot[color=red, mark=o] coordinates {
    (50, 1.8) (100, 2.3) (200, 3.1) (500, 4.5) (1000, 6.9)
};
\addlegendentry{\DSAIN{}}
\end{axis}
\end{tikzpicture}
...
Moreover, you are required to declare funding (financial activities supporting the submitted work) and competing interests (related financial activities outside the submitted work).

\textbf{Funding:} [To be completed by authors---declare all funding sources]

\textbf{Competing Interests:} [To be completed by authors---declare any conflicts]}
...
For the honest updates, define $\sigma_{\mathcal{H}}^2 = \frac{1}{|\mathcal{H}|}\sum_{i \in \mathcal{H}} \norm{\Delta_i - \bar{\Delta}_{\mathcal{H}}}^2$. By concentration of the geometric median \citep{minsker2015geometric}:
\begin{equation}
