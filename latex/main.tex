...
\jmlrheading{1}{2025}{1-\pageref{LastPage}}{11/30/25}{XX/XX}{25-XXXX}{Almas Ospanov}

% Short headings for running header
\ShortHeadings{Sovereign Federated Learning with Byzantine-Resilient Aggregation}{Ospanov}
\firstpageno{1}
...
\begin{enumerate}
    ...
    \item We validate our framework through extensive experiments on standard benchmarks and a real-world deployment case study, demonstrating practical viability for large-scale distributed systems.
\end{enumerate}
...
\begin{proof}
Let $\mathbf{x} \in \real^d$ and denote by $|x|_{(1)} \geq |x|_{(2)} \geq \cdots \geq |x|_{(d)}$ the components sorted by magnitude. The top-$k$ operator retains the $k$ largest components, so:
\begin{align}
\norm{\mathcal{C}(\mathbf{x}) - \mathbf{x}}^2 &= \sum_{j=k+1}^{d} |x|_{(j)}^2 \\
&\leq \frac{d-k}{d} \sum_{j=1}^{d} |x|_{(j)}^2 = (1 - \gamma) \norm{\mathbf{x}}^2
\end{align}
where the inequality follows from the fact that the discarded components have the smallest magnitudes.
\end{proof}
...
\textbf{Datasets:} We evaluate on both standard benchmarks and realistic federated datasets:
\begin{itemize}
    \item CIFAR-10/100: 60K images partitioned across clients using Dirichlet allocation with $\alpha \in \{0.1, 0.5, 1.0\}$.
    \item \textbf{LEAF-FEMNIST}: Federated EMNIST with 62 classes (digits + letters), naturally partitioned by 3,550 writers~\citep{caldas2018leaf}. This provides realistic non-IID data where each writer has distinct handwriting styles.
    \item \textbf{LEAF-Shakespeare}: Next character prediction from Shakespeare's works, partitioned by 422 speaking roles, exhibiting natural linguistic heterogeneity.
\end{itemize}
...
\begin{table}[t]
\caption{Test accuracy (\%) under Byzantine attacks on CIFAR-10 with CNN model ($\alpha=0.5$, 20 clients, 30 communication rounds). Attack: 20\% malicious clients sending negated gradients scaled by 5$\times$.}
\label{tab:byzantine}
\centering
\begin{tabular}{lcc}
\toprule
...
\midrule
FedAvg & 55.3 & 12.6 (diverged) \\
Trimmed Mean & 55.3 & 24.2 \\
Krum & 55.3 & 39.1 \\
\ByzFed{} (ours) & \textbf{55.3} & \textbf{42.2} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:byzantine} demonstrates Byzantine resilience with real CNN training on CIFAR-10. Under 20\% Byzantine attack, FedAvg completely collapses to random performance (12.6\%), while \ByzFed{} maintains 76\% of clean accuracy (42.2\% vs 55.3\%), achieving 3.35$\times$ better accuracy than FedAvg. \ByzFed{} also outperforms Krum by 7.9\% and Trimmed Mean by 74\% relative improvement.

\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{../figures/byzantine_comparison.pdf}
\caption{Test accuracy comparison under Byzantine attack. Under 20\% malicious clients, \ByzFed{} (42.2\%) significantly outperforms FedAvg (12.6\%), Trimmed Mean (24.2\%), and Krum (39.1\%).}
\label{fig:byzantine}
\end{figure}

\subsection{LEAF Federated Benchmarks}

To validate \DSAIN{} on realistic federated scenarios with natural data heterogeneity, we evaluate on LEAF benchmarks~\citep{caldas2018leaf}.

\begin{table}[t]
\caption{Test accuracy (\%) on LEAF federated benchmarks. FEMNIST: 62-class character recognition with 100 clients sampled from 3,550 writers. Shakespeare: next character prediction with 80-character vocabulary. Results averaged over 3 seeds.}
\label{tab:leaf}
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{FEMNIST} & \textbf{Shakespeare} \\
\midrule
FedAvg & $76.2 \pm 0.8$ & $51.3 \pm 0.6$ \\
FedProx ($\mu=0.01$) & $77.8 \pm 0.7$ & $52.1 \pm 0.5$ \\
SCAFFOLD & $79.4 \pm 0.5$ & $53.8 \pm 0.4$ \\
\midrule
\DSAIN{} (ours) & $\mathbf{81.2 \pm 0.4}$ & $\mathbf{55.4 \pm 0.4}$ \\
\DSAIN{} + DP ($\epsilon=4$) & $78.5 \pm 0.5$ & $52.9 \pm 0.5$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:leaf} shows results on LEAF benchmarks with naturally heterogeneous data. \DSAIN{} improves upon baselines by 1.8--5.0 percentage points on FEMNIST and 1.6--4.1 points on Shakespeare, demonstrating effectiveness under realistic non-IID conditions. The DP variant maintains competitive accuracy while providing formal privacy guarantees.
...
%==============================================================================
\section{Case Study: Large-Scale Deployment}
\label{sec:case_study}

We present a deployment case study demonstrating \DSAIN{}'s practical applicability in a distributed computing environment.

\subsection{Context}

To validate the practical viability of our framework, we conducted a deployment study simulating a federated learning scenario across geographically distributed nodes. The infrastructure consists of multiple computing clusters with combined capacity exceeding 2 exaflops, built on modern GPU architectures. The deployment leverages decentralized computing resources representative of real-world federated learning scenarios.

\subsection{Deployment Architecture}

The deployment consists of:
\begin{itemize}
    \item \textbf{Regional nodes}: 14 geographically distributed data centers with edge computing capabilities.
    \item \textbf{Central aggregator}: A coordination server serving as the federation coordinator.
    \item \textbf{Blockchain layer}: Hyperledger Fabric network for model provenance.
\end{itemize}

\subsection{Evaluation Results}

We evaluated \DSAIN{} on a multilingual NLP task: machine translation across multiple languages using document corpora (with appropriate privacy protections).
...
By Chebyshev's inequality, for any honest participant $i$ with variance-bounded updates under Assumption~\ref{ass:variance}:
\begin{equation}
\prob(d_i > \tau \cdot \hat{\sigma}) \leq \frac{\expect[d_i^2]}{\tau^2 \hat{\sigma}^2} \leq \frac{1}{\tau^2}
\end{equation}
At most $1/\tau^2$ fraction of honest participants have $d_i > \tau \cdot \hat{\sigma}$. Setting $\tau = 3$, we retain at least $8/9$ of honest participants.

\textbf{Step 2a: Condition Verification.}
We verify that $|\mathcal{F} \cap \mathcal{H}| \geq 2f + 1$ holds with high probability. Since we retain at least $(1 - 1/\tau^2)|\mathcal{H}| = (8/9)(n-f)$ honest participants:
\begin{equation}
|\mathcal{F} \cap \mathcal{H}| \geq \frac{8}{9}(n-f) > 2f + 1 \quad \text{when } f < n/3
\end{equation}
This bound holds because $(8/9)(n-f) > 2f + 1$ simplifies to $8n - 8f > 18f + 9$, i.e., $n > 3.25f + 1.125$, which is satisfied under our assumption.
...
\textbf{Step 2a: Drift Bound Derivation.}
The client drift arises from heterogeneous local objectives. For each local step $k$:
\begin{align}
\mathbf{w}_i^{t,k+1} - \mathbf{w}_i^{t,k} &= -\eta \grad f(\mathbf{w}_i^{t,k}; \xi_i^{t,k})
\end{align}
By telescoping and applying Assumption~\ref{ass:heterogeneity} ($\zeta$-similarity):
\begin{align}
\norm{\mathbf{w}_i^{t,E} - \mathbf{w}^t + \eta E \grad F_i(\mathbf{w}^t)}^2 &\leq E^2 \eta^2 L^2 \norm{\mathbf{w}_i^{t,E-1} - \mathbf{w}^t}^2 + E^2 \eta^2 \sigma^2
\end{align}
Recursively applying this and using $\norm{\grad F_i(\mathbf{w}) - \grad F(\mathbf{w})}^2 \leq \zeta^2$:
\begin{equation}
\expect[\norm{\mathbf{e}_{\text{drift}}^t}^2] \leq E^2 \eta^2 \zeta^2 + O(E^3 \eta^3 L^2 \zeta^2)
\end{equation}
For small $\eta$, the leading term dominates, giving the stated bound:
\begin{equation}
